...
func dashboardConfigFrom(cfg runtimeConfig) dashboardConfig {
	return dashboardConfig{
		...
		PrimaryContextBudgetTokens: cfg.PrimaryContextBudgetTokens, PrimaryOutputTokenLimit: cfg.PrimaryOutputTokenLimit,
		TextFallbackModels: cfg.TextFallbackModels,
...
func managementHTML(cfg runtimeConfig) (page string) {
	defer func() {
		page = strings.NewReplacer(
			...
			"视觉模型只负责把图片转成不可信的事实文本；主任务仍由首选文本模型完成。历史图片按需恢复，长对话超过阈值后自动压缩。", "自动适配 OpenAI Chat、Responses 与 Claude Messages。旧图按需恢复；历史压缩按完整语义轮次和工具事务选择边界，长对话达到阈值后再复用摘要检查点。",
			"当前图片完整识别；旧图片默认只保留短摘要，明确引用时恢复详情。", "当前图片完整识别；未引用旧图合并为固定归档标记，明确引用时按上限恢复。",
			"<strong>纯文本轮：</strong>完全跳过视觉链，只有达到压缩阈值时才多一次摘要调用。", "<strong>纯文本轮：</strong>保持原始工具历史；只有达到压缩阈值时才建立或复用摘要检查点。",
			"按需恢复减少非图片续问中的上下文；自动压缩仅在接近阈值时触发。", "未引用旧图不再逐张解码；摘要边界不会切断工具调用与结果；检查点仅在仍接近预算时创建或更新。",
			"无关轮只注入这段短摘要，不保存或重传原图。", "无关轮只注入一条固定归档标记，不解码、不哈希、不重传原图。",
			"达到阈值后把较早历史压缩为 untrusted 摘要，保留最近轮次原文。", "首次达到阈值建立可持久复用的 untrusted 摘要检查点；后续仅在新增历史再次逼近预算时更新。",
			"这些最近消息保持原文，不进入摘要。", "工具调用与结果按完整语义轮次和工具事务保留；摘要边界不会切断调用与结果。",
			"历史图片短摘要字符数", "历史图片归档标记字符上限",
			"历史短摘要不计入；恢复详情时仍受总容量限制。", "固定归档标记不计入；恢复详情时仍受总容量限制。",
			"按需恢复 · 无关轮短摘要", "按需恢复 · 固定归档标记",
			"图片轮首先调用；建议选择延迟较低、截图 OCR 稳定的模型。", "图片轮首先调用；按实际截图的准确率和完成耗时排序。",
			"视觉模型按顺序逐个尝试，所有识别子请求强制 low 思考，并清除外层 system、tools 和 reasoning 配置。", "视觉模型按顺序逐个尝试；识别子请求固定 low 思考，清除外层 system、tools 和 reasoning 配置，并且不设置输出 token 上限。",
			...
			`<div class="field"><label>可取消识别超时（秒）</label><input type="number" min="1" name="vision_timeout_seconds"><small>Host 返回 stream ID 后开始计时；例如填写 30 表示已建立视觉流最多识别 30 秒。首包前仍受 Host ABI 限制。</small></div><div class="field"><label>取消确认等待（秒）</label><input type="number" min="1" max="120" name="vision_cancel_grace_seconds"><small>仅在 stream_close 后等待 Host 确认结束；例如填写 25 表示最多等待 25 秒。正常请求不增加此延迟，未确认时不启动备用模型。</small></div>`,
			...
			"越小越省上下文；越大保留更多历史细节。", "用于规划摘要检查点大小；摘要请求会按该目标增加容差并显式设置输出预算。",
			...
			"记录将展示缓存命中、视觉回退、自动压缩、文本回退和完整耗时。", "记录将展示历史图片处理、上下文预检、摘要检查点、模型回退和完整耗时。",
		).Replace(page)
	}()
...
<section id="settings" class="view"><form id="configForm"><div class="config-layout"><div class="config-sections"><div class="config-section"><div class="section-title"><h3>1. 对外入口与文本模型链</h3><p>只对外暴露一个虚拟模型名；文本链与视觉链模型不能重复。</p></div><div class="fields"><div class="field full"><label>对外模型名</label><input name="combo_model"><small>客户端只使用这一个名称，例如 glm-5.2-vision-combo。</small></div><div class="field"><label>首选文本模型</label><select name="primary_model" data-model></select><small>无图和识图后的最终回答都首先交给它。</small></div><div class="field"><label>文本备用模型 1</label><select name="text_fallback_0" data-model></select><small>首选模型在输出任何内容前失败时使用。</small></div><div class="field"><label>文本备用模型 2</label><select name="text_fallback_1" data-model></select><small>可留空；不会调用视觉模型来生成最终答案。</small></div><div class="field"><label>文本备用模型 3</label><select name="text_fallback_2" data-model></select><small>流式回答已输出后不会切换，避免混合两份答案。</small></div></div></div>
<div class="config-section"><div class="section-title"><h3>2. 视觉候选链</h3><p>视觉模型按顺序逐个尝试，所有识别子请求强制 low 思考，并清除外层 system、tools 和 reasoning 配置。</p></div><div class="fields"><div class="field"><label>首选视觉模型</label><select name="vision_primary_model" data-model></select><small>图片轮首先调用；建议选择延迟较低、截图 OCR 稳定的模型。</small></div><div class="field"><label>备用视觉模型 1</label><select name="vision_backup_model_1" data-model></select><small>首选报错、无有效内容、明确截断或由 CPA Host 判定失败后调用。</small></div><div class="field"><label>备用视觉模型 2</label><select name="vision_backup_model_2" data-model></select><small>可留空。</small></div><div class="field"><label>备用视觉模型 3</label><select name="vision_backup_model_3" data-model></select><small>可留空，最多四级视觉链。</small></div><div class="field"><label>视觉模型上下文上限</label><input type="number" name="vision_context_limit"><small>当前四个候选共享此值；256K 建议填写 262144 或实际可用上限。</small></div><div class="field"><label>单模型软延迟预算（秒）</label><input type="number" name="vision_timeout_seconds"><small>用于观测慢调用。CPA Host 暂不支持取消；插件会接收成功的迟到结果，避免后台遗留请求与重复计费。</small></div><div class="field"><label>附近文字预算（tokens）</label><input type="number" name="vision_input_token_budget"><small>只带当前用户问题附近文字，不再把完整 1M 会话发送给 256K 视觉模型。</small></div></div></div>
<div class="config-section"><div class="section-title"><h3>3. 主上下文、历史图片与 v0.7 安全压缩</h3><p>v0.7 只在完整语义轮次和工具事务之间压缩；摘要边界不会切断调用与结果。</p></div><div class="preset-bar"><div><strong>v0.7 推荐长上下文方案</strong><span>1,000,000 上限 · 900,000 输入预算 · 64,000 输出上限 · 820,000 触发 · 12,000 摘要 · 保留 8 个最近语义单元</span></div><button class="button compact" id="applyV07Preset" type="button">应用推荐值</button></div><div class="budget-readout" id="budgetReadout"></div><div class="fields"><div class="field"><label>主模型上下文上限</label><input type="number" min="100000" step="1000" name="primary_context_tokens"><small>当前 GLM-5.2 配置为 1,000,000；这是理论输入窗口。</small></div><div class="field"><label>主模型输入工作预算</label><input type="number" min="100000" step="1000" name="primary_context_budget_tokens"><small>推荐 900,000；正文、工具 Schema 和协议估算都计入该输入安全线。</small></div><div class="field"><label>最终文本输出上限</label><input type="number" min="1024" step="1000" name="primary_output_token_limit"><small>推荐 64,000；客户端较小值保留，缺失或过大时由插件注入或封顶。</small></div><div class="field"><label>历史图片策略</label><select name="history_attachment_mode"><option value="onDemand">按需恢复（推荐）</option><option value="retain">每轮完整保留</option></select><small>未引用旧图使用固定归档标记，明确追问时才恢复详情。</small></div><div class="field"><label>历史图片归档标记字符上限</label><input type="number" min="120" max="4000" name="history_attachment_compact_chars"><small>推荐 600；该值限制固定标记，不保存或重传原图。</small></div><div class="field"><label>单轮最多恢复旧图片数</label><input type="number" min="1" max="16" name="history_attachment_restore_max_attachments"><small>推荐 2；只恢复最近旧图，并受本轮总图片数限制。</small></div><div class="field"><label>并发识图数</label><input type="number" min="1" max="8" name="max_concurrent_extractions"><small>推荐 2；只影响多图并发，单图仍按候选链顺序回退。</small></div><div class="field switch-field full"><div><label>启用 v0.7 安全历史压缩</label><small>达到阈值后建立可复用摘要检查点；边界不会切断 call/result，未完成工具调用保留原文。</small></div><input class="switch" type="checkbox" name="auto_compression_enabled"></div><div class="field"><label>压缩触发阈值（tokens）</label><input type="number" min="10000" step="1000" name="auto_compression_threshold_tokens"><small>推荐 820,000；比 900,000 工作预算低 80,000。</small></div><div class="field"><label>摘要规划大小（tokens）</label><input type="number" min="1000" step="1000" name="auto_compression_target_tokens"><small>推荐 12,000；摘要请求会按该目标增加容差并显式设置输出预算。</small></div><div class="field"><label>保留最近语义单元</label><input type="number" min="1" max="64" name="auto_compression_keep_recent_turns"><small>推荐 8；按真实用户轮次和完整工具事务处理，不按普通 JSON 条数切割。</small></div><div class="field"><label>压缩模型</label><select name="auto_compression_model" data-model></select><small>留空使用首选文本模型；首包前失败后才尝试文本备用链。</small></div></div></div>
